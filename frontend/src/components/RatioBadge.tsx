...
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
  ...
  export function RatioBadge({ ratio, showValue = true, showIcon = false }: { ratio: number; showValue?: boolean; showIcon?: boolean }) {
      ...
      <div className="flex items-center gap-2">
        <Tooltip>
          <TooltipTrigger asChild>
            <span 
              className={`inline-flex items-center justify-center min-w-[28px] px-2 py-1 rounded-md text-xs font-medium border leading-none ${colorClass}`}
            >
              {showIcon && (
                <svg
              ...
              {showValue && (
                <span className="ml-1.5 text-[10px] opacity-75 font-mono tabular-nums leading-none">
                  {ratio.toFixed(1)}
                </span>
              )}
            </span>
