...
function ScheduleSpeedLimitInput({ label, limit, onChange }: Readonly<{ label: string; limit: number; onChange: (limit: number) => void }>) {
...
export function BandwidthSchedules({ workerId }: Readonly<{ workerId: string }>) {
  ...
  const toggleDay = (day: number) => setForm(current => ({ ...current, days_of_week: current.days_of_week.includes(day) ? current.days_of_week.filter(item => item !== day) : [...current.days_of_week, day].sort((left, right) => left - right) }));
  const save = async () => {
  ...
  const persistReorder = async (reordered: BandwidthSchedule[]) => {
    setItems(reordered);
    setReordering(true);
  ...
  const moveSchedule = (index: number, offset: number) => {
  ...
  const grid = useMemo(() => days.map((day, dayIndex) => ({ day, slots: items.filter(item => item.enabled && item.days_of_week.includes(dayIndex)) })), [days, items]);
  ...
  return <div className="space-y-5 p-4">
    ...
    <p className="text-xs text-muted-foreground">{t("workers.schedules.reorderHint", "Drag rules to reorder priority.")} {t("workers.schedules.keyboardReorderHint", "Use the move controls to reorder with a keyboard.")} {items.length >= MAX_SCHEDULES && t("workers.schedules.maxReached", "Maximum of 5 schedules per worker.")}</p>
