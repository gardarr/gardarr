...
export function DeleteTorrentModal({ 
  ...
  const handleConfirm = () => {
    onConfirm(purge);
    setPurge(false); // Reset para próxima vez
  };

  const handleClose = () => {
    setPurge(false); // Reset ao fechar
    onClose();
  };
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <DialogTitle className="text-lg">Remover Torrent</DialogTitle>
              <DialogDescription>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          >
            {purge ? "Remover Tudo" : "Remover Torrent"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
